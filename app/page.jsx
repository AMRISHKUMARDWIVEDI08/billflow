...
import { encodeFunctionData, isAddress, parseEventLogs, parseGwei, parseUnits, recoverMessageAddress } from "viem";
...
function encodeRequest(value) { return btoa(unescape(encodeURIComponent(JSON.stringify(value)))).replaceAll("+", "-").replaceAll("/", "_").replaceAll("=", ""); }
function decodeRequest(value) { try { const padded = value.replaceAll("-", "+").replaceAll("_", "/") + "===".slice((value.length + 3) % 4); return JSON.parse(decodeURIComponent(escape(atob(padded)))); } catch { return null; } }
function requestMessage(payload) { return `FlowProof Payment Request v1\n${JSON.stringify(payload)}`; }
function shortAddress(value) { return value ? `${value.slice(0, 6)}…${value.slice(-4)}` : "—"; }
async function signWithInjectedWallet(message, address) { const ethereum = typeof window !== "undefined" ? window.ethereum : undefined; if (!ethereum?.request) return null; const result = await ethereum.request({ method: "personal_sign", params: [message, address] }); return typeof result === "string" ? result : null; }
...
export default function Home() {
  ...
  const { data: balance, refetch: refetchBalance } = useBalance({ address, token: USDC, chainId: arcTestnet.id, query: { enabled: Boolean(address) } });
  ...
  useEffect(() => {
    ...
    const loadRequest = async () => {
      const params = new URLSearchParams(window.location.search); const encoded = params.get("request"); if (!encoded) return;
      const decoded = decodeRequest(encoded);
      if (!decoded?.amount || !decoded?.recipient || !decoded?.signature || decoded.token?.toLowerCase() !== USDC.toLowerCase() || decoded.network !== "arc-testnet" || !isAddress(decoded.recipient)) { if (!cancelled) setNotice("invalid or unsigned payment request."); return; }
      try {
        const signedPayload = { version: decoded.version, network: decoded.network, token: decoded.token, amount: decoded.amount, recipient: decoded.recipient, reference: decoded.reference, description: decoded.description, createdAt: decoded.createdAt };
        ...
        if (!cancelled && trusted) { setRequest(decoded); setRequestTrusted(true); setRequestUrl(window.location.href); setMode("pay"); }
        else if (!cancelled) setNotice("payment request signature does not match its recipient.");
      } catch { if (!cancelled) setNotice("payment request signature could not be verified."); }
    };
    loadRequest(); return () => { cancelled = true; };
  }, []);

  const amountUnits = useMemo(() => { try { return amount ? parseUnits(amount, 6) : 0n; } catch { return null; } }, [amount]);
  ...
  const createRequest = async () => {
    ...
    setBusy(true); setNotice("opening your wallet to sign the payment request…");
    try {
      const payload = { version: 1, network: "arc-testnet", token: USDC, amount, recipient: address, reference: invoiceNumber.trim(), description: description.trim(), createdAt: new Date().toISOString() };
      let signature;
      try { signature = await signMessageAsync({ message: requestMessage(payload) }); }
      catch (primaryError) { signature = await signWithInjectedWallet(requestMessage(payload), address); if (!signature) throw primaryError; }
      const signedPayload = { ...payload, signature }; const url = `${window.location.origin}/?request=${encodeRequest(signedPayload)}`;
      setRequestUrl(url); setRequest(signedPayload); setRequestTrusted(true); setMode("pay"); setNotice("signed payment request created. changing its terms invalidates the signature.");
    } catch (error) { setNotice(error?.shortMessage || error?.message || "request signature was rejected or the wallet did not respond."); }
  ...
  const payRequest = async () => {
    ...
    if (chainId !== arcTestnet.id) {
      ...
      catch (error) { return setNotice(error?.shortMessage || "switch to Arc Testnet was rejected."); }
    }
    if (address.toLowerCase() === request.recipient.toLowerCase()) return setNotice("for a real payment test, use a different recipient wallet address.");
    try {
      setBusy(true); setNotice("preparing the Arc USDC transaction…");
      const value = parseUnits(request.amount, 6);
      if (!publicClient) throw new Error("Arc RPC is not available yet.");
      const currentGasPrice = await publicClient.getGasPrice();
      const maxFeePerGas = currentGasPrice > parseGwei("20") ? currentGasPrice : parseGwei("20");
      const data = encodeFunctionData({ abi: transferAbi, functionName: "transfer", args: [request.recipient, value] });
      const hash = await writeContractAsync({ account: address, chainId: arcTestnet.id, address: USDC, abi: transferAbi, functionName: "transfer", args: [request.recipient, value], data, maxFeePerGas, maxPriorityFeePerGas: 0n });
      setNotice(`transaction submitted to Arc: ${shortAddress(hash)}. waiting for confirmation…`);
    } catch (error) { setNotice(error?.shortMessage || error?.message || "transaction could not be submitted. check wallet and Arc Testnet connection."); }
    finally { setBusy(false); }
  };

  useEffect(() => { if (isConfirmed) { refetchBalance(); setNotice("transaction confirmed on Arc. you can now verify the payment evidence."); } }, [isConfirmed, refetchBalance]);

  const verifyPayment = async () => {
    setNotice(""); setVerification(null);
    ...
    try {
      setBusy(true); const hash = verifyHash.trim();
      ...
      const usdcTransfers = logs.filter((log) => log.address.toLowerCase() === USDC.toLowerCase()); const transfer = usdcTransfers.length === 1 ? usdcTransfers[0] : null;
      const expectedAmount = parseUnits(request.amount, 6);
      const amountMatch = Boolean(transfer && transfer.args.value === expectedAmount); const recipientMatch = Boolean(transfer && transfer.args.to?.toLowerCase() === request.recipient.toLowerCase()); const senderMatch = Boolean(transfer && transfer.args.from?.toLowerCase() === tx.from.toLowerCase()); const tokenMatch = Boolean(transfer && transfer.address.toLowerCase() === USDC.toLowerCase()); const directUsdcCall = tx.to?.toLowerCase() === USDC.toLowerCase(); const success = receipt.status === "success"; const networkMatch = tx.chainId === arcTestnet.id;
      const signedPayload = { version: request.version, network: request.network, token: request.token, amount: request.amount, recipient: request.recipient, reference: request.reference, description: request.description, createdAt: request.createdAt }; const signer = await recoverMessageAddress({ message: requestMessage(signedPayload), signature: request.signature }); const requestAuthentic = signer.toLowerCase() === request.recipient.toLowerCase();
      ...
      setVerification({ verified, success, networkMatch, directUsdcCall, tokenMatch, amountMatch, recipientMatch, senderMatch, requestAuthentic, sender: transfer?.args.from || tx.from, recipient: transfer?.args.to || tx.to, amount: transfer ? Number(transfer.args.value) / 1_000_000 : null, blockNumber: receipt.blockNumber.toString(), hash });
    ...
    } catch (error) { setNotice(error?.shortMessage || error?.message || "could not verify this transaction on Arc."); }
    finally { setBusy(false); }
  };
